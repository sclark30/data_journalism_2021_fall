...
**Answer**
Dataframe describing MD county vaccinations by date.
```{r}
...
**Answer**
There is an NA column at the bottom with a total of 201.
...
**Answer**
Would NA represent the data that isn't reported? Or something is wrong in the function to where it's not updating daily.
...
**Answer**
Over the past 7-10days, the total of fully vaccinated people is leveling out.
```{r}
...
**Answer**
Bar chart showing the rate of people getting vaccinated daily since April. The data is now starting to level out as more and more people are fully vaccinated.
```{r fig.width=9}
ggplot() +
...
**Task** Run the following code and put the value of `sub` in the answer.
**Answer**
The number of people in Prince George county who are fully vaccinated has decreased at a high rate since April 2021.
...
**Answer** The number of COVID cases per county in MD has increased at an expontential rate since July 2020.
...
**Task** Run the following code and describe what the resulting chart shows
**Answer**
The number of COVID cases in MD counties is leveling out in recent months.
```{r}
...
**Answer** The map is showing the number of reported COVID cases per county in MD on 11/15/2021
