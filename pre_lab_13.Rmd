...
**Answer** A dataframe displaying whether each state won the Trump of Clinton vote, what percentage of the state voted Trump and what percentage of small businesses in the state are receiving a PPP loan as well as the average amount of money each employee is receiving. 
```{r}
...
**Answer** There is a cluster of dots which show red states that are receving loans at a higher rate.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Less of a correlation showing the amount of money each small business employee receives is directly connected to a red state.
```{r}
...
**Answer** There is a correlation between a state's businesses receiving ppe and the amount of money each employee receives but it is not a super strong correlation. 
```{r}
reveal_data %>%
...
**Answer** An output describing a 95 percent confidence level that our predictions are correct and not just statistical errors.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** An output describing a weaker correlation from our hypothesis.
```{r}
...
**Answer** A table describing the average amount of ppp received by businesses and employees in Clinton and Trump states.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** A t value of 1 and p value of .2295, meaning there is a pattern in the data.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** A t value of 3 and a p value of .002 meaning there is a promise for something more in this data set that we should look in to.
```{r}
