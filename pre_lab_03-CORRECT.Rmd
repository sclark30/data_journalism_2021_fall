...
# Load the tidyverse   
library(tidyverse)
NOTE: I am sorry this is late. I had to unexpecedtly go into work, so I am abruptly finishing this at 1am. I'll take the late points off, but I wnated to get it done before class. Also I got stuck on question 11, because the libraries will not load, but I can complete it once we talk about it in class. 
```
...
**Answer** One more paising issues, see "problems()" for details
...
**Answer** Yes, it says no loop for break/next, jumping to top level.
...
**Answer**:When franchise_name is NA there are 195865 loans. I tried to change the code to make the name "subway" show up, but I wasn't sure where I need to make the exact change. I tried under group_by and summarise, but those did not work. 
...
**Answer**: THere are 135073 when the name is forgiveness_date and the same is true when it is forgiveness_amount.
...
**Answer**: There are 135073. I can conclude that from the loan data, that when loans are forgiven, the date that they are forgiven on results in the same data, because the two are essential with each other. For every forgiven loan amount, you must have a specified date of when that loan was forgiven. 
```{r}
ppp_maryland_loans %>%
