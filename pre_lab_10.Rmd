...
We are reading the Maryland PPP dataset into a new dataframe called md_counties and then we are grouping the data by project_county_name to organize it and then counting how many approved applications their are per county.
...
Reading Maryland PPP into the md_counties dataframe and then grouping by project county name and then counting the number of PPP applications and filtering them by 6, to find the extraneous records that don't initially show up.
...
The new fips_code column specifies a five digit FIPS code for every county.
...
It has both the fips codes for the counties, and the number of approved apps per county.
```{r}
...
It looks cleaner than the previous map and has zoomed in to just Maryland.
...
**Answer** Describe the changes from the previous map
Only thing is I added in that exact formula and then it showed up weird on my map and said there was an error. 
...
https://www.datawrapper.de/_/5SavO/
